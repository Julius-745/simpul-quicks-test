...
export const QuicksMenu = () => {
    ...
    const menuList : Menu[] = [
        {
            ...
            icon: <Button
                    ...
                    onClick={() => openMenu(0)}
                    >
                    <ChatIcon width="30" height="30" color={activeMenu === 0 ? "white" : "#8785FF"} />
                </Button>,
            menu: <ChatMenu/>
        },
        {
    ...
    return (
        ...
        <HStack display={"flex"} gap={5} flexDir={activeMenu !== 1  ? "row-reverse" : "row"}>
        ...
        {menuList.map((item, idx) => (
            <Stack 
                ...
                transform={open ? `translateX(-${(idx+1)*10})` : `translateX(${(idx+1)*10}px)`} 
                opacity={open ? 1 : 0}>
            <Text display={activeMenu !== -1 ? "none" : "inline"}>{item.name}</Text>
